...
class PumpSyncImplementation @Inject constructor(
    ...
    override fun syncBolusWithTempIdMedLink(timestamp: Long, amount: Double, temporaryId: Long, type: BS.Type?, pumpId: Long?, pumpType: PumpType, pumpSerial: String): Boolean {
        if (!confirmActivePump(timestamp, pumpType, pumpSerial)) return false
        val bolus = BS(
            timestamp = timestamp,
            amount = amount,
    ...
    override fun lastGlucoseValue(): Optional<GV> {
        val glucoseValue = persistenceLayer.getLastGlucoseValue()
        return glucoseValue?.let { Optional.of(it) } ?: empty()
    }
